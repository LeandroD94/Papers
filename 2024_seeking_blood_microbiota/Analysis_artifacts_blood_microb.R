...
ggsave(file="Results/Abundances/TOP_5_phyla_pdf.pdf",width=5.85,height=3.7, dpi=300)
dev.off()
...
ggplot(data=tabella, aes(x=Sample_code, y=Abundance, fill=Genus)) +
  ...
  labs(x="", y="Abundance %", 
       fill="",
       title = "Five most abundant genera (after removing E.coli from ctrls)",
       caption = " 'Others' includes every genus below rank 5 ") +
  coord_flip() # usefull to flip everything with just a click!
...
ggplot(data=tabella, aes(x=Sample_code, y=Abundance, fill=Phylum)) +
  ...
  labs(x="", y="Abundance %", 
       fill="",
       title = "Five most abundant phyla (after removing E.coli from ctrls)",
