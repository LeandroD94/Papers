...
################### ANALYSING LEFSE RESULTS (PS , every comparison) ###########################

dir.create("Results/PS/PICRUSt2_LEfSe")
dir.create("Results/PS/PICRUSt2_LEfSe/only_over_2_5")
...
### Starting the loop
for( x in dir("QIIME/PICRUST2_LEFSE/")[grepl("Result_",dir("QIIME/PICRUST2_LEFSE/"))] ) {
  file_name<-gsub("Result_LEFSE_PS_in","",x) # to save it
  file_name<-gsub(".res","",file_name)
  
  Significative_functions_LEFSE<- read.delim(paste0("QIIME/PICRUST2_LEFSE/",x), header=FALSE)
  ...
  write.xlsx(Significative_functions_LEFSE, file=paste0("Results/PS/PICRUSt2_LEfSe/Signif_pathways_KEGG_",file_name,".xlsx"), col.names = T, showNA = F, row.names = F)
  
  # modifing names for the plot
  Significative_functions_LEFSE$Pathway<-gsub("&beta;-","", Significative_functions_LEFSE$Pathway, fixed = T)
  Significative_functions_LEFSE$Pathway[Significative_functions_LEFSE$Pathway_ID=="ko00071"] <- "Fatty acid degradation"
  # Instead of "fatty acid metabolism", see https://www.genome.jp/dbget-bin/www_bget?pathway+ko00071
  ...
  one_out_of_two<-Significative_functions_LEFSE$Class_with_highest_mean[1]
  Significative_functions_LEFSE$logLDA_score[Significative_functions_LEFSE$Class_with_highest_mean==one_out_of_two]<- -Significative_functions_LEFSE$logLDA_score[Significative_functions_LEFSE$Class_with_highest_mean==one_out_of_two]
  ...
  ggplot(data=Significative_functions_LEFSE, aes(y=Pathway, x=as.numeric(logLDA_score), 
    ...
    geom_text(aes(x= 0, label=Pathway), 
              hjust = ifelse(Significative_functions_LEFSE$Class_with_highest_mean==one_out_of_two,1,0), size=3.2) +
    scale_fill_manual(values=c("Ntg"="deepskyblue","G93A"="red", "C57Ola"="chartreuse3","129Sv"="coral2" )) +
    theme_classic(base_size = 16) +
  ...
  ggsave(filename = paste0("Results/PS/PICRUSt2_LEfSe/BarPlot_",file_name,"_KEGG_every_result.png"), width = 7, height = 5, dpi = 300)
  
  #plotting only results over 2.5
  if( length(Significative_functions_LEFSE[abs(Significative_functions_LEFSE$logLDA_score)>2.5, ][[1]]) >0 ){
    Significative_functions_LEFSE_3<-Significative_functions_LEFSE[abs(Significative_functions_LEFSE$logLDA_score)>2.5,]
    ggplot(data=Significative_functions_LEFSE_3, aes(y=Pathway, x=as.numeric(logLDA_score), fill=Class_with_highest_mean)) +
      geom_bar(stat = "identity", width = 1, color="black" ) + labs(x="log LDA score", y="", fill="") +
      geom_text(aes(x= 0, label=Pathway), hjust = ifelse(Significative_functions_LEFSE_3$Class_with_highest_mean==one_out_of_two,1,0), size=3.2) +
      scale_fill_manual(values=c("Ntg"="deepskyblue","G93A"="red", "C57Ola"="chartreuse3","129Sv"="coral2" )) +
      theme_classic(base_size = 16) +
      scale_x_continuous(breaks = c(-3,-2,-1, 1, 2, 3))+
      theme(panel.grid.major.x = element_line(color = "grey", size = 0.3)) +
    ...
    ggsave(filename = paste0("Results/PS/PICRUSt2_LEfSe/only_over_2_5/BarPlot_",file_name,"_KEGG_only_over3.png"), width = 7, height = 3, dpi = 300)
  }
  
  suppressWarnings( rm(Significative_functions_LEFSE, Significative_functions_LEFSE_3, a, file_name) )
