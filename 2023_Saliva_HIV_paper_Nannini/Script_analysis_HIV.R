...
par(mar=c(6,4,4,4), cex.lab=0.9, cex.main=1.35, cex.sub=1.2)
plot(c,main="Community structure using Hellinger distance on sqrt proportional ASVs",
     sub="IR = light green     INR = dark green")
dev.off()
...
sample_data(data.sqrt_prop)$Responders<-gsub("1","IR",sample_data(data.sqrt_prop)$Responders)
sample_data(data.sqrt_prop)$Responders<-gsub("0","INR",sample_data(data.sqrt_prop)$Responders)
DistBC = phyloseq::distance(data.sqrt_prop, method = "euclidean")
ordBC = ordinate(data.sqrt_prop, method = "PCoA", distance = DistBC)
eigval<-ordBC$values$Eigenvalues # to get eigen values of every PC
eigval<- round((eigval/sum(eigval))*100, 1) # to get variance explained by every PC
plot_ordination(data.sqrt_prop, ordBC, color = "Responders") +
  scale_color_manual(values = c("INR" = "chartreuse4", "IR"="chartreuse")) +
  geom_point(size=3) + theme_classic(base_size = 14) +stat_ellipse() +
  labs(title="PCoA with Hellinger distance  (saliva samples)",
...
Table_tot$Responders<-gsub("0","INR",Table_tot$Responders)
Table_tot$Responders<-gsub("1","IR",Table_tot$Responders)
Table_tot$Responders<-factor(Table_tot$Responders, levels=c("INR","IR") )
ggplot(Table_tot, aes(x= Bacteria, y=Abundance, color=Responders, fill=Responders)) + 
  ...
  geom_point(position=position_dodge(width=0.6), size=1, alpha= 0.9) +
  geom_point(position=position_dodge(width=0.6), size=2.5, alpha= 0.55) +
  theme_classic(base_size = 11) +
  theme(strip.text.x=element_text(size=12,colour="black")) + 
  scale_color_manual(values=c("INR"="chartreuse4","IR"="chartreuse2")) +
  guides( color=guide_legend(nrow=1), fill="none" ) +
  theme(legend.margin=margin(-25, 0, 0, 0), legend.position="bottom", 
        legend.key.size=unit(0.8,"cm"), 
        legend.text=element_text(size=13),
...
Significative_functions_LEFSE$Pathway<-Descriptions$description
Significative_functions_LEFSE$Pathway_ID<-Descriptions$pathway
Significative_functions_LEFSE$Class_with_highest_mean<-gsub("Not responders","INR",Significative_functions_LEFSE$Class_with_highest_mean)
Significative_functions_LEFSE$Class_with_highest_mean<-gsub("Responders","IR",Significative_functions_LEFSE$Class_with_highest_mean)
head(Significative_functions_LEFSE, n=4)
...
# inverting the values of a group to make a simmetric plot
Significative_functions_LEFSE$logLDA_score[Significative_functions_LEFSE$Class_with_highest_mean=="INR"]<- -Significative_functions_LEFSE$logLDA_score[Significative_functions_LEFSE$Class_with_highest_mean=="INR"]

# removing the Calvin Benson cycle (humans are not plant!)
Significative_functions_LEFSE<-Significative_functions_LEFSE[Significative_functions_LEFSE$Pathway!=" Calvin-Benson-Bassham cycle ", ]
...
ggplot(data=Significative_functions_LEFSE, aes(y=Pathway, x=as.numeric(logLDA_score), 
  ...
  geom_bar(stat = "identity", width = 1, color="black" ) + labs(x="log LDA score", y="", fill="") +
  geom_text(aes(x= 0, label=Pathway), 
            hjust = ifelse(Significative_functions_LEFSE$Class_with_highest_mean=="INR",1,0), size=3.2) +
  scale_fill_manual(values=c("INR"="chartreuse4","IR"="chartreuse")) +
  theme_classic(base_size = 16) +
  scale_x_continuous(breaks = c(-3,-2,-1, 1, 2, 3))+
  theme(panel.grid.major.x = element_line(color = "grey", size = 0.3)) +
  theme(axis.text.y=element_blank(), 
