...
fill_color_19<-c("wheat3","deeppink","deepskyblue2","black","darkgreen","yellow2","brown","firebrick3","springgreen2","coral","bisque","darkmagenta","darkslategray3","blue", "yellow4","red","cyan","gold","green2","gray","violet") 
...
ggsave(file="Results/General_data_analysis/PHA_Accumulators_in_both_N_and_Plim_without_R2.png",width=7.25,height=5.5,dpi=300)
dev.off()
...
###################### ABUNDANCES BAR PLOT _ N_lim Only  BUT WITHOUT FIRST DAYS ##########################

# TOP 5 Phylum stacked bar plot (no inoculum)
suppressWarnings(rm(top, others, tabella))
...
ggsave(file="Results/N_lim_reactor/Abundances/TOP5_phyla_abundances_WITHOUT_DAYS_1_AND_3.png",width=7,height=6.5, dpi=300) 
dev.off()

# means of TOP5 phyla (no inoculum)
write.xlsx(file = "Results/N_lim_reactor/Abundances/TOP5_phyla_WITHOUT_DAYS_1_AND_3_averages.xlsx", row.names = F,
           cbind.data.frame("mean"=as.numeric(apply(otu_table(prune.dat_top),1,mean)), "phylum"= as.data.frame(tax_table(prune.dat_top))[["Phylum"]]))


# TOP 10 Genera (no inoculum)
suppressWarnings(rm(top, others, tabella))
...
ggsave(file="Results/N_lim_reactor/Abundances/TOP10_genera_abundances_WITHOUT_DAYS_1_AND_3.png",width=7,height=6.5, dpi=300) 
dev.off()

# means of TOP10 genera (no inoculum)
write.xlsx(file = "Results/N_lim_reactor/Abundances/TOP_10_genera_WITHOUT_DAYS_1_AND_3_Averages.xlsx", row.names = F,
           cbind.data.frame("mean"=as.numeric(apply(otu_table(prune.dat_top),1,mean)), "genus"= as.data.frame(tax_table(prune.dat_top))[["Genus"]]))
...
fill_color_19<-c("wheat3","deeppink","darkmagenta","coral","cyan","yellow2","brown","firebrick3","springgreen2","violet","bisque","gray","darkslategray3","blue", "yellow4","red","darkgreen","gold","violet","green","deepskyblue2") 
...
con <- file("Results/N_lim_reactor/Time_series_in_N_lim/README_Only_certain_genera_have_been_tested.txt")
sink(con, append = T)
...
ggsave(file="Results/N_lim_reactor/Time_series_in_N_lim/Genera_CLR_positively_correlated_with_time.png",width=7,height=5, dpi=300) 
dev.off()
...
ggsave(file="Results/N_lim_reactor/Time_series_in_N_lim/Genera_CLR_negatively_correlated_with_time.png",width=7,height=5, dpi=300) 
dev.off()
