...
ggplot(data=tabella, aes(x=Abundance, y=Experiment_day, fill=Phylum)) + theme_classic(base_size =14) + 
  ...
  theme(legend.position="bottom",legend.margin = margin(2,0,5,0)) +
  guides(fill=guide_legend(nrow=3)) +
  labs(y="Experiment day", x="Percent abundance",
       fill="",
       title = "Five most abundant phyla", caption = " 'Others' includes every phylum below rank 5 ")
...
order_time<-tabella$Sampling_date[c(grep("/04",tabella$Sampling_date),grep("/05",tabella$Sampling_date),grep("/06",tabella$Sampling_date))] # months
tabella$Sampling_date<-factor(tabella$Sampling_date, levels = unique(order_time))
ggplot(data=tabella, aes(x=Abundance, y=Experiment_day, fill=Genus)) + 
...
ggsave(file="Results/P_lim_reactor/R1_versus_R2/Abundances/TOP10_genera_abundances.png",width=7,height=10.3, dpi=300) 
dev.off()
...
plot_1 +
  ...
  labs(title= "Differently abundant genera", y="Percent abundance", fill=" ", x="")
ggsave(filename = "Results/P_lim_reactor/R1_versus_R2/DA_DESeq2/Plot_genera_DeSeq2_Type.png", width = 10.8, height = 7, dpi=300)
# again but with name
plot_1 +
  theme_bw() +
  geom_text(aes(label=Sample_name), size=2.3, color="black") +
  geom_line(aes(group=Sampling_date), size=0.2, col="darkgray") +
  labs(title= "Differently abundant genera", y="Percent abundance", fill="Sample_Type", x="")
ggsave(filename = "Results/P_lim_reactor/R1_versus_R2/DA_DESeq2/Plot_genera_DeSeq2_Type_with_name.png", width = 10.8, height = 7, dpi=300)
dev.off()
...
plot_3<-ggplot(tabella_3, aes(x= Xaxis, y=Abundance, fill=Sample_Type)) +
  ...
  theme(panel.grid.minor.y= element_blank()) +
  theme(legend.margin=margin(0, 0, 0, 0), legend.position="none") +
  labs(title= "Differently abundant families and orders", y="Percent Abundance", fill=" ", x="") +
  geom_line(aes(group=Sampling_date), size=0.2) 
...
png(filename = "Results/P_lim_reactor/R1_versus_R2/DA_DESeq2/Plot_TOTAL_DESeq2_Type_no redundants.png", width = 2750, height = 1500, res=300)
...
head_plot<-plot_4 + # refining first plot with the title
  labs(title= "Differently abundant families, orders, class and phyla", y="Percent abundance", x="") + 
  theme(plot.title = element_text(size=12.5)) + 
  geom_text(aes(label=Sample_name), size=2, color="black")
plot3_bis<- plot_3 +   geom_text(aes(label=Sample_name), size=2, color="black") +
...
ggsave(file="Results/P_lim_reactor/Time_series_in_R1/LOG_RATIO_counts_correlated_with_time/6_Genera_most_positively_correlated.png",width=7,height=5, dpi=300) 
dev.off()
...
ggplot(data=tabella, aes(y=Abundance, x=Experiment_day, color=Genus)) +
  ...
  labs(x="Experiment day",
       ...
       color="",
       title = "the 6 genera most negatively correlated with time (R1)\n according to spearman correlation on CLR counts")
ggsave(file="Results/P_lim_reactor/Time_series_in_R1/LOG_RATIO_counts_correlated_with_time/6_Genera_most_negatively_correlated.png",width=7,height=5, dpi=300) 
dev.off()
