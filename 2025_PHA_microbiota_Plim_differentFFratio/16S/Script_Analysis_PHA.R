...
r<-rarecurve(t(as(otu_table(data_with_unsat.genus),"matrix")),
             step=150,label=F, 
             ylab = "Genera", xlab= "Reads amount",
             col="darkgray"
             )
# evalslopes(r,sample_names(data_with_unsat.genus),lim=0.001,cex=1.05)
evalslopes(r, paste0(sample_data(data_with_unsat.genus)$Sample_Type,"_",sample_data(data_with_unsat.genus)$Experiment_day,"th"), 
                    lim=0.001,cex=0.65)
dev.off()
rm(r)
...
ggplot(data=tabella, aes(y=Abundance, x=Experiment_day, fill=Genus)) + 
  ...
  theme( strip.text = element_text(size=12),
         ...
         plot.title = element_blank(),
         plot.margin = margin(4,1,1,0),
         legend.key.height  = unit(0.35, "cm"),
         legend.key.width = unit(0.25, "cm"),
         legend.key.spacing.x = unit(0.35, "cm"),
         legend.text = element_text ( size = 10 , face="italic"),
         legend.position="bottom",
         legend.margin = margin(-1,0,1,-14.5)
         ) +
...
ggsave(file="Results/Abundances/TOP_genera_abundances.png",height =5.2,width = 5.95, dpi=300) 
dev.off()
...
lista_PHA<-c(lista_PHA,"SM1A02")  # DOI:10.1016/j.envres.2024.118722
...
fill_color_PHA<-c("chocolate","white","lightblue4","yellow2",
                  "lightcoral","lightgreen","grey35","deeppink",
                  "cyan","coral3","wheat4","grey90","darkmagenta","brown4",
                  "orange","yellow3", "blue1","bisque3",
                  "indianred", "chartreuse3","darkslategray3","yellow4","black",
...
ggplot(data=tabella, aes( x=Experiment_day, y=Abundance, fill=Genus)) +
  ...
  theme( strip.text = element_text(size=11),
         ...
         panel.grid.major.y = element_line( colour="gray40", linewidth = 0.12) ,
         panel.spacing.x = unit(0.75,"pt"),
         strip.clip = "off",
         axis.title.x = element_text(size=11),
         ...
         legend.position="bottom",
         legend.margin = margin(-2,0,1,-15.5)
  ) +
  guides(fill=guide_legend(nrow=5)) +
  labs(x="Experiment day", y="Percent abundance", 
       fill="",
...
ggsave(file="Results/Abundances/PHA_accumulators_along_the_time.png",width=5.45,height=4.9,dpi=300)
dev.off()
...
mix_alpha$ID<-gsub("F","1/2", mix_alpha$ID )
...
ggplot(data=mix_alpha, aes(y=value, x=Time_axisX, color=ID)) +
  ...
  theme(axis.text.x = element_text(angle=25, size=7.5, vjust=1, hjust=1),
        ...
        legend.title = element_text(size=10.5, margin = margin(0,0,0,35) ) ,
        legend.text = element_text(size=9.5 ) ,
        legend.spacing.x = unit(0.15,"cm"),
        legend.margin = margin(-5,15,0,-25),
...
ggsave(file="Results/Alfa_diversity_GENUS_along_the_time.png", width = 3.85,height =3.4, dpi=300)
...
sample_data(data.prop.labels)$F_F_ratio<-gsub("R2","1/2", sample_data(data.prop.labels)$F_F_ratio )
...
sample_data(data.prop.labels)$Experiment_day2[ ! sample_data(data.prop.labels)$Experiment_day2 %in% c(1, 22 ,  38, 63, 81, 95 ) ] <- ""
sample_data(data.prop.labels)$Experiment_day2[ sample_data(data.prop.labels)$Experiment_day2 =="1" ] <- "1  "
sample_data(data.prop.labels)$Experiment_day2[ sample_data(data.prop.labels)$Experiment_day2 =="22" ] <- "22\n "
sample_data(data.prop.labels)$Experiment_day2[ sample_data(data.prop.labels)$Experiment_day2 =="81" ] <- "81\n \n \n \n "
sample_data(data.prop.labels)$Experiment_day2[ sample_data(data.prop.labels)$Experiment_day2 =="63" & sample_data(data.prop.labels)$F_F_ratio=="1/2" ] <- "    63"
sample_data(data.prop.labels)$Experiment_day2[ sample_data(data.prop.labels)$Experiment_day2 =="95" & sample_data(data.prop.labels)$F_F_ratio=="1/2" ] <- "      95"
...
plot_ordination(data.sqrt_prop, ordBC, color = "F_F_ratio") +
  ...
  theme(
        ...
        legend.title = element_text(size=12, margin = margin(0,0,0,35) ),
        legend.text = element_text(size=9.5), 
        legend.spacing.x = unit(0.25,"cm"),
        legend.margin = margin(-5,15,0,-20),
        legend.direction = "horizontal",
        legend.position = "bottom",
  ) +
...
data_pruned2 <- subset_samples(data_pruned, Experiment_state %in% "Almost_stabilized" )
...
for( t in c("Genus","Family","Class","Order","Phylum") ){
  ...
  res<-res[res$baseMean > 75, ] # arbitrary threshold to avoid the most noisy result
  res
...
tabella_g$Experiment_day2[ !tabella_g$Experiment_day2 %in% c("63","81","95") ] <- ""


plot_1<-ggplot(tabella_g, aes(x= Xaxis, y=Abundance, fill=Sample_Type)) +
  theme_classic(base_size = 8.6) +
  scale_color_manual(values = c("R2"="deepskyblue2","R1"="coral")) +
  ...
  geom_point(aes(color=Sample_Type), size=0.2, alpha=1 ) +
  geom_point(aes(color=Sample_Type), size=1.65, alpha=0.35) +
  theme(strip.text.x=element_text(size=5,colour="gray10",
                                  lineheight = 0.75,
                                  margin = margin(1.5,0,1.5,0, "pt")
  ),
  strip.background = element_rect(color = "black", size = 0.65),        
  panel.spacing.x = unit(0.11, "line"),
...
plot_1 +
  ...
  guides(fill="none", color="none")+
    geom_text(aes(label=Experiment_day2), size=1.65, color="gray20", alpha= 1,
              hjust=ifelse(tabella_g$Sample_Type=="R1",1.3,-0.3)
    )
ggsave(filename = "Results/DA_DESeq2/Plot_genera_DeSeq2_Type_AFTER_EXP_STARTED.png",
       width = 4, height = 4.45, dpi=500)
# again but with name
plot_1 +
