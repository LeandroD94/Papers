...
suppressWarnings( dir.create("RNA_PHA_Results/Differentially_expressed_genes") )
...
############ SIGNIF DIFFERENCES S vs F in PHA REACTORS  (DESeq2) ################
...
################### FOCUSING ON THE MOST DIFFERENTS ONES ###################
...
to_plot$Reactor<-gsub("R2","2/1", to_plot$Reactor)
to_plot$Reactor<-factor(to_plot$Reactor, levels = c("1/1","2/1"))
...
ggplot(to_plot, aes(x= Reactor, y=value, fill=Reactor)) +
  ...
  scale_color_manual(values = c("1/1"="coral","2/1"="deepskyblue")) +
  facet_wrap2( .~Var1,
  ...
  labs(title="Twentyfive most significant increased functions in reactor R1",
       y="Percent counts",
       color="F/F ratio", fill="F/F ratio"
  )
ggsave(filename = "RNA_PHA_Results/Differentially_expressed_genes/DESeq2_Twentyfive_most_increased_RNA_in_R1.png", width = 6.2, height = 5.2, dpi=300)
dev.off()
...
write.csv2(file="RNA_PHA_Results/Differentially_expressed_genes/DESeq2_Twentyfive_most_increased_RNA_in_R1_infos.csv", table_res, row.names=T, quote=F)
...
ggplot(to_plot, aes(x= Reactor, y=value, fill=Reactor)) +
  ...
  theme(strip.text.x=element_text(size=7.5,colour="black"),
        ...
        legend.margin=margin(-6, 0, -2, 0),
        legend.position = "bottom") +
  labs(title="Twentyfive most significant increased functions in reactor R2",
       y="Percent counts",
       color="F/F ratio", fill="F/F ratio"
  )
ggsave(filename = "RNA_PHA_Results/Differentially_expressed_genes/DESeq2_Twentyfive_most_increased_RNA_in_R2.png", width = 6.2, height = 5.2, dpi=300)
dev.off()
...
write.csv2(file="RNA_PHA_Results/Differentially_expressed_genes/DESeq2_Twentyfive_most_increased_RNA_in_R2_infos.csv", table_res, row.names=T, quote=F)
...
ggsave(filename = "RNA_PHA_Results/Differentially_expressed_genes/DESeq2_PHA_related.png", width = 4.6, height = 5, dpi=300)
dev.off()
...
write.csv2(file="RNA_PHA_Results/Differentially_expressed_genes/DESeq2_PHA_related_infos.csv", table_res, row.names=T, quote=F)
