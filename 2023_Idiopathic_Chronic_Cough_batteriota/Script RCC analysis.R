...
sample_data(data)$Condition<-factor(sample_data(data)$Condition, levels=c("Healthy","RCC")) # decides the order in plots
...
p1<-plot_ordination(data.sqrt_prop, ordBC, color = "Condition", shape="Sampling_type") +
  scale_color_manual(values=c("RCC"="coral","Healthy"="chartreuse")) +
...
plot_ordination(data.sqrt_prop, ordBC, color = "Condition", shape = "Sampling_type") +
  scale_color_manual(values=c("Healthy"="chartreuse","RCC"="coral")) +
  geom_point(size=4.2) + theme_classic(base_size = 14) + stat_ellipse(size=0.2) + 
...
plot_ordination(data.sqrt_prop, ordBC, color = "Condition", shape = "Sampling_type") +
  scale_color_manual(values=c("Healthy"="chartreuse","RCC"="coral")) +
  geom_point(size=4.2, alpha=0.65) + theme_classic(base_size = 14) + stat_ellipse(size=0.2) +
  geom_line(aes(group=Subject), color= "grey", size= 0.15) +
  labs(title="PCoA with Hellinger distance (euclidean on Hellinger transformed ASV)", 
       color="Condition", x=paste("PC1: ",eigval[1],"% variation"), y=paste("PC2: ",eigval[2],"% variation"),
       caption="the lines are connecting the same sample")
...
plot_ordination(data.sqrt_prop, ordBC, color = "Condition", shape = "Sampling_type") +
  scale_color_manual(values=c("Healthy"="chartreuse","RCC"="coral")) +
  geom_point(size=4.2, alpha= 0.68) + theme_classic(base_size = 14) + stat_ellipse(size=0.2) +
  geom_line(aes(group=Subject), color= "grey", size= 0.15) +
  labs(title="PCoA with Hellinger distance (euclidean on Hellinger transformed Genera)", 
       color="Condition", x=paste("PC1: ",eigval[1],"% variation"), y=paste("PC2: ",eigval[2],"% variation"),
       caption="the lines are connecting the same sample")
ggsave(file="Results/General_Beta_div/PCoA_Beta_diversity_Hellinger_on_Genera_points.png", width = 9.5, height = 6, dpi=300)
# ggsave(file="PCoA_General_poster.png", width = 6.5, height = 5.5, dpi=300)
...
ggsave(file="Results/Saliva/Abundances/TOP_5_phyla_Saliva.png",width=8,height=5, dpi=300)
dev.off()
...
ggsave(file="Results/Saliva/Abundances/TOP_5_genera_Saliva.png",width=8,height=5,dpi=300)
dev.off()
...
ggsave(file="Results/Saliva/Abundances/TOP_8_genera_Saliva.png",width=8,height=5,dpi=300)
# poster version
ggplot(data=tabella, aes(x=Subject, y=Abundance, fill=Genus)) + facet_grid(cols= vars(Condition),scales = "free_x", space = "free_x") +
  geom_bar(stat="identity", position="stack") + theme_classic(base_size =14) +
  scale_fill_manual(values=fill_color_8) +
  ...
  labs(x="Patients", y="Relative abundance", title = "Eigth most abundant genera in Saliva")
ggsave(file="TOP_8_genera_Saliva_poster.png",width=6.5,height=3.8,dpi=500)
dev.off()
...
ratio_fb[ratio_fb$Condition=="RCC","Ratios_Mean"]<-Ratios_Mean["RCC"]
...
ratio_fb[ratio_fb$Condition=="RCC","Ratios_st_err"]<-Ratios_st["RCC"]/sqrt(length(which(ratio_fb$Condition=="RCC")))
...
ratio_fb$Condition<-factor(ratio_fb$Condition, levels = c("RCC","Healthy"))
...
ggplot(data=ratio_fb, aes(x=Condition, fill=Condition,y=Ratio)) +
  theme_bw(base_size =9) +
  scale_fill_manual(values = c("RCC"="coral","Healthy"="chartreuse")) +
  facet_grid(.~Condition, scales = "free_x", space="free")+
  theme(panel.spacing.x = unit(20,"pt"))+
  scale_x_discrete (expand = c(0.2,0.3) ) +
  scale_y_continuous(breaks=c(0, 0.5, 1, 1.5, 2, seq(2,10,0.5))) +
...
ggsave(file="Results/Saliva/Abundances/Ratio_Firmi_Bacteroi/Ratios_FB_boxplot_median_IQR.png",width=3.5,height=2.8, dpi=300) 
dev.off()
...
colors <- gsub("RCC","coral",colors) #orange
...
pAlpha + geom_boxplot(data=pAlpha$data, aes(x=Condition, y=value, color=NULL), alpha= 0) + theme_classic2() +  # alpha 0 overwrites the original black color
  scale_color_manual(values = c("Healthy"="chartreuse", "RCC"="coral")) +
  labs(x="Condition", title="Alpha diversity between RCC and Healthy (Saliva)") +
...
ggsave(file="Results/Saliva/Alfa_diversity_with_Mann_Withn_Wilcox_Saliva.png", width = 6,height =5, dpi=300)
...
plot_ordination(data.sqrt_prop, ordBC, color = "Condition") +
  scale_color_manual(values=c("Healthy"="chartreuse","RCC"="coral")) +
  geom_point(size=4.5, alpha=0.78) + theme_classic(base_size = 13.4) + stat_ellipse(size=0.2) + 
...
plot_ordination(data.sqrt_prop, ordBC, color = "Condition") +
  scale_color_manual(values=c("Healthy"="chartreuse","RCC"="coral")) +
  geom_point(size=4.5, alpha=0.8) + theme_classic(base_size = 13) + stat_ellipse(size=0.2) + 
  #geom_text(aes(label=sample_names(data.sqrt_prop)), color="black", size=1.5, show.legend = FALSE) +
  labs(title="PCoA with Hellinger distance \n computed on Saliva subset", 
       color="Condition", x=paste("PC1: ",eigval[1],"% variation"), y=paste("PC2: ",eigval[2],"% variation"), subtitle = paste("PERMANOVA Pr(>F) =",perm_g_H))
ggsave(file="PCoA_Saliva_poster.png", width = 5, height = 3.5, dpi=500)
...
RCC<-subset_samples(data.venn, Condition=="RCC")
RCC<-as.character(tax_table(prune_taxa(taxa_sums(RCC)>0, RCC))[,"Genus"])


ONLY_IN_RCC<- RCC[! RCC %in% Healthy]
ONLY_IN_RCC<- paste(ONLY_IN_RCC, collapse = ", ")
head(ONLY_IN_RCC)

ONLY_IN_Healthy<- Healthy[! Healthy %in% RCC]
ONLY_IN_Healthy<- paste(ONLY_IN_Healthy, collapse = ", ")
head(ONLY_IN_Healthy)


x<-list(Healthy=Healthy,RCC=RCC)
...
suppressWarnings(rm(ONLY_IN_RCC, ONLY_IN_Healthy, x, con, RCC, Healthy, data.venn, who))
...
for( t in c("Genus","Family","Class","Order","Phylum") ){
  ...
  DE<-DESeq(DEseq_data)
  res<-results(DE, contrast= c("Condition", "Healthy", "RCC"))
  ...
  if(length(res$log2FoldChange)>0){ # if there are results...
    ...
    # write.csv2(r, file=paste0("Results/Saliva/DA_DESeq2/DA_",t,"_ratio_Healthy_vs_RCC_Saliva.csv"), row.names = F, quote=F, na = "")
    r_level<-r
    r_level[, "Taxon"]<- rep(t)
...
ggplot(Table_tot, aes(x= Bacteria, y=Abundance, fill=Condition)) + 
  ...
  scale_fill_manual(values=c("Healthy"="chartreuse","RCC"="coral")) +
...
DE_plot<-ggplot(Table_tot2, aes(x= Bacteria, y=Abundance, fill=Condition)) + 
  ...
  theme(legend.margin=margin(-25, 0, 0, 0), legend.position="bottom", 
        ...
        axis.text.x = element_text(angle = 30, vjust=1, hjust=1, size=11.5), 
...
DE_plot
ggsave(filename = "Results/Saliva/DA_DESeq2/DA_Condition_WITHOUT_redundants_Saliva.png", width = 10, height = 7, dpi=300)
dev.off()
...
ggplot(Table_tot2, aes(x= Bacteria, y=Abundance, fill=Condition)) + 
  facet_grid(~factor(Taxa,levels = c("Phylum","Class","Order","Family","Genus")), scales = "free_x", space="free") +
  geom_boxplot(width=0.7) + theme_bw(base_size = 15) +
  scale_fill_manual(values=c("Healthy"="chartreuse","RCC"="coral")) +
  theme(strip.text.x=element_text(size=13,colour="black")) + 
  guides( fill=guide_legend(nrow=1) ) +
  theme(legend.margin=margin(-32, 0, 0, 0), legend.position="bottom", 
        legend.key.size=unit(0.85,"cm"), 
        legend.text=element_text(size=16),
        axis.text.x = element_text(angle = 25, vjust=1.03, hjust=1, size=11.5), 
        axis.text.y = element_text(size=9.05), 
        plot.title= element_text(size=17),
        panel.grid.minor.y= element_blank(),
        plot.margin =  margin(t=5,r=4,b=5, l=15) ) +  
  scale_x_discrete(expand=c(-0.2, 1)) +
  scale_y_sqrt(breaks=c(0.1, 0.5, 1,seq(2,32,2),34.5,37,39.5)) +
  labs(title= "Differently abundant Taxa (Saliva)", y="Proportional Abundance", 
       fill="", x="")
ggsave(filename = "DA_Saliva_poster.png", width = 8, height = 5.5, dpi=500)
...
circo + labs(title="RCC vs Healthy \n Saliva DA taxa log2foldchange\n")
dev.off()
...
length(SCFA$Sample_Code[! is.na(SCFA$Sample_Code)]) == 8  # TRUE --> any sample cutted avay (there are FFA values regarding 8 RCC)
...
ggplot(corr, aes(x = corr$DA_Genera, y = corr$SCFA, fill = corr$Corr)) +
  ...
  theme_bw(base_size=12) +
  theme(axis.text.x=element_text(angle = -25, hjust = 0, size= 10)) +
  guides(fill= guide_colourbar(title ="rho")) +
  geom_text(aes(label= Sign), color= "white", size =7) +
...
ggsave(file="Results/Saliva/DA_genera_vs_SCFA_using_only_RCC.csv.png", dpi=300, width = 6, height = 6)
write.csv2(corr, file="Results/Saliva/DA_genera_vs_SCFA_using_only_RCC.csv", quote = F, na="", row.names = F)

# better version
corr2<-corr[corr$SCFA!="X2.MethylButyrric", ]
corr2$DA_Genera<-factor(corr2$DA_Genera, levels = sort(unique(as.character(corr2$DA_Genera)))) # following alphabetical order
original_levels<-gsub(".","-",levels(corr2$SCFA), fixed=T)
corr2$SCFA<-gsub(".","-",corr2$SCFA, fixed=T)
corr2$SCFA<-factor(corr2$SCFA, levels = original_levels)
ggplot(corr2, aes(x = DA_Genera, y = SCFA, fill = Corr)) +
  geom_tile(color = "white", lwd = 0.5, linetype = 1) +
  scale_fill_gradient2(low="blue", mid = "white", high="red", limits=c(-1,1)) +
  theme_bw(base_size=12) +
  theme(axis.text.x=element_text(angle = 25, hjust = 1, vjust=1.05, size= 10)) +
  guides(fill= guide_colourbar(title ="rho")) +
  geom_text(aes(label= Sign), color= "white", size =8.5, vjust=0.7) +
  labs(title = "", y= "Serum DA FFA", x= "Saliva DA Genera", 
       caption= "\n adjusted p-value (BH) lower than 0.05 are displayed through * sign") +
  theme(plot.title = element_text(size=18)) +
...
ggsave(file="Results/Saliva/DA_genera_vs_FFA_using_only_RCC_WITHOUT_MethButyrric.png", dpi=300, width = 6, height = 6)
# poster version
ggplot(corr2, aes(x = DA_Genera, y = SCFA, fill = Corr)) +
  geom_tile(color = "white", lwd = 0.5, linetype = 1) +
  scale_fill_gradient2(low="blue", mid = "white", high="red", limits=c(-1,1)) +
  theme_bw(base_size=12) +
  theme(axis.text.x=element_text(angle = 22, hjust = 1, vjust=1.05, size= 10)) +
  theme(axis.text.y=element_text(size= 10.5)) +
  guides(fill= guide_colourbar(title ="rho")) +
  geom_text(aes(label= Sign), color= "white", size =8.5, vjust=0.7) +
  labs(title = "",
       y= "Serum DA FFA", x= "Saliva DA Genera") +
  theme(plot.title = element_text(size=18),
        plot.margin =  margin(t=-10,r=2,b=2, l=2),
        legend.margin = margin(t=0,r=2,b=2, l=-4),
        legend.text = element_text(size=10), 
        legend.key.height= unit(1.5, 'cm'),
        legend.key.width= unit(0.5, 'cm'),
        legend.title = element_text(size=14))
ggsave(file="DA_genera_vs_SCFA_using_only_RCC_poster.png", dpi=500, width = 5, height = 4.5)
...
Significative_functions_LEFSE$logLDA_score[Significative_functions_LEFSE$Class_with_highest_mean=="RCC"]<- -Significative_functions_LEFSE$logLDA_score[Significative_functions_LEFSE$Class_with_highest_mean=="RCC"]
...
ggplot(data=tabella, aes(x=Subject, y=Abundance, fill=Phylum)) + facet_grid(cols= vars(Condition),scales = "free_x", space = "free_x") + 
  ...
  theme(axis.text.x=element_text(angle=35, vjust=1, hjust=1), 
        legend.key.size = unit(0.4, "cm"),legend.text = element_text ( size = 12 )) + 
  theme(legend.position="bottom") + guides(fill=guide_legend(nrow=2)) +
  labs(x="Patients", y="Relative abundance", title = "Five most abundant phyla (Stool)", 
       caption = " 'Others' includes every phylum below rank 5 ")
ggsave(file="Results/Stool/Abundances/TOP_5_phyla_Stool.png",width=8,height=5, dpi=300)
dev.off()
...
# tabella$Condition<-gsub("RCC"," ",tabella$Condition)    # if it is needed to rename
...
ggplot(data=tabella, aes(x=Subject, y=Abundance, fill=Genus)) + facet_grid(cols= vars(Condition),scales = "free_x", space = "free_x") +
  ...
  theme(legend.position="bottom", legend.margin = margin(3,0,0,0)) + guides(fill=guide_legend(nrow=3)) + 
  labs(x="Patients", y="Relative abundance", title = "Eigth most abundant genera", caption = " 'Others' includes every genus below rank 8 ")
ggsave(file="Results/Stool/Abundances/TOP_8_genera_Stool.png",width=8,height=5,dpi=300)
# poster version
ggplot(data=tabella, aes(x=Subject, y=Abundance, fill=Genus)) + facet_grid(cols= vars(Condition),scales = "free_x", space = "free_x") +
  geom_bar(stat="identity", position="stack") + theme_classic(base_size =14) +
  scale_fill_manual(values=fill_color_8) +
  ...
  theme(legend.position="bottom", legend.margin = margin(-5,0,0,0)) +
  guides(fill=guide_legend(nrow=3)) + 
  labs(x="Patients", y="Relative abundance", title = "Eigth most abundant genera in Stool")
ggsave(file="TOP_8_genera_Stool_poster.png",width=6.5,height=3.8,dpi=500)
...
ggsave(file="Results/Stool/Abundances/Ratio_Firmi_Bacteroi/Ratios_FB_boxplot_median_IQR.png",width=3.5,height=2.8, dpi=300) 
dev.off()
...
pAlpha + geom_boxplot(data=pAlpha$data, aes(x=Condition, y=value, color=NULL), alpha=0) + theme_classic2() + 
  scale_color_manual(values = c("Healthy"="chartreuse", "RCC"="coral")) +
  labs(x="Condition", title="Alpha diversity between RCC and Healthy (Stool)") +
  guides(fill="none", color="none") + theme(axis.text.x= element_text(angle=0, vjust=0, hjust=0.5, size=10)) +
  stat_compare_means(aes(group = Condition), label="p.format", method = "wilcox.test", label.x= 1.4, size=3.5, 
                     label.y.npc = "top", vjust=-0.5, hjust=0.3) +
  theme(panel.grid.major.y = element_line(size=0.4), panel.grid.minor.y = element_line(size=0.25) , 
        axis.title.x = element_text(vjust = -1))
ggsave(file="Results/Stool/Alfa_diversity_with_Mann_Withn_Wilcox_Stool.png", width = 6,height =5, dpi=300)
...
plot_ordination(data.sqrt_prop, ordBC, color = "Condition") +
  scale_color_manual(values=c("Healthy"="chartreuse","RCC"="coral")) +
  geom_point(size=4) + theme_classic(base_size = 14) + stat_ellipse(size=0.2) + 
  geom_text(aes(label=sample_names(data.sqrt_prop)), color="black", size=3, show.legend = FALSE) +
...
plot_ordination(data.sqrt_prop, ordBC, color = "Condition") +
  scale_color_manual(values=c("Healthy"="chartreuse","RCC"="coral")) +
  geom_point(size=4) + theme_classic(base_size = 14) + 
  geom_text(aes(label=sample_names(data.sqrt_prop)), color="black", size=3, show.legend = FALSE) +
  labs(title="PCoA with Hellinger distance (euclidean on Hellinger transformed ASV)\n computed on Stool subset",
       color="Condition", x=paste("PC1: ",eigval[1],"% variation"), y=paste("PC2: ",eigval[2],"% variation"), subtitle = paste("PERMANOVA Pr(>F) =",perm_ASV_H))
...
plot_ordination(data.sqrt_prop, ordBC, color = "Condition") +
  scale_color_manual(values=c("Healthy"="chartreuse","RCC"="coral")) +
  geom_point(size=4, alpha=0.5) + theme_classic(base_size = 14) + stat_ellipse(size=0.2) +
  labs(title="PCoA with Hellinger distance (euclidean on Hellinger transformed ASV)\n computed on Stool subset", 
       color="Condition", x=paste("PC1: ",eigval[1],"% variation"), y=paste("PC2: ",eigval[2],"% variation"), subtitle = paste("PERMANOVA Pr(>F) =",perm_ASV_H))
...
plot_ordination(data.sqrt_prop, ordBC, color = "Condition") +
  ...
  labs(title="PCoA with Hellinger distance \n computed on Stool subset", 
       color="Condition", x=paste("PC1: ",eigval[1],"% variation"), y=paste("PC2: ",eigval[2],"% variation"), subtitle = paste("PERMANOVA Pr(>F) =",perm_g_H))
ggsave(file="PCoA_Stool_poster.png", width = 5, height = 3.5, dpi=500)
...
for( t in c("Genus","Family","Class","Order","Phylum") ){
  ...
  if(length(res$log2FoldChange)>0){ # if there are results...
    ...
    r$Species<-NULL
    assign(paste(t,"results",sep="_"), r)
    # write.csv2(r, file=paste0("Results/Stool/DA_DESeq2/DA_",t,"_ratio_Healthy_vs_RCC_Stool.csv"), row.names = F, quote=F, na = "")
    r_level<-r
    r_level[, "Taxon"]<- rep(t)
    Res_tot<-rbind.data.frame(Res_tot,r_level)
    ### single box plots
...
DE_plot
ggsave(filename = "Results/Stool/DA_DESeq2/DA_Condition_WITHOUT_redundants_Stool.png", width = 10.5, height = 7, dpi=300)
dev.off()
...
row.names(SCFA)<-SCFA$Sample_Code
SCFA<-SCFA[row.names(target), ]
length(SCFA$Sample_Code[! is.na(SCFA$Sample_Code)]) == 7  # TRUE --> any sample cutted avay (there are FFA values regarding 8 RCC but no stool for one of them)
SCFA <- SCFA[! is.na(SCFA$Sample_Code), ]
...
ggplot(corr, aes(x = corr$DA_Genera, y = corr$SCFA, fill = corr$Corr)) +
  ...
  theme(legend.text = element_text(size=10), legend.key.height= unit(2, 'cm'),
        legend.key.width= unit(1, 'cm'), legend.title = element_text(size=12))
ggsave(file="Results/Stool/DA_genera_vs_SCFA_using_only_RCC.csv.png", dpi=300, width = 6, height = 6)
write.csv2(corr, file="Results/Stool/DA_genera_vs_SCFA_using_only_RCC.csv", quote = F, na="", row.names = F)
...
ggplot(corr2, aes(x = DA_Genera, y = SCFA, fill = Corr)) +
  ...
  guides(fill= guide_colourbar(title ="rho")) +
  geom_text(aes(label= Sign), color= "white", size =8.5, vjust=0.7) +
  labs(title = "", y= "Serum DA FFA", x= "Stool DA Genera", 
       caption= "\n adjusted p-value (BH) lower than 0.05 are displayed through * sign") +
  theme(plot.title = element_text(size=18)) +
  theme(legend.text = element_text(size=10), 
        legend.key.height= unit(2, 'cm'),
        legend.key.width= unit(0.8, 'cm'),
        legend.title = element_text(size=13))
ggsave(file="Results/Stool/DA_genera_vs_FFA_using_only_RCC_WITHOUT_NAs.csv.png", dpi=300, width = 6, height = 6)
...
head(SCFA, n=2)
SCFA$Condition<-gsub("ICC","RCC",SCFA$Condition)
...
ggplot(data=table_SCFA2, mapping=aes(x=variable, y=value, fill=Condition)) +
  scale_fill_manual(values = c("Healthy"="chartreuse","RCC"="coral")) +
  theme_bw(base_size = 15) +
  theme( axis.text.x = element_text(angle=28, size = 9, vjust=1, hjust = 1),
         axis.text.y = element_text(size = 7),
         axis.title.y = element_text(size = 11),
         panel.grid.minor.y = element_line(linewidth = 0.1),
         panel.grid.major.y = element_line(linewidth = 0.32),
         title = element_text(size = 10.5)) +
  scale_y_log10(breaks= c(0.1, 0.2, 0.5, 1, 2.5, 5, 10, 25, 50,
                          100, 250, 500)) +
  scale_x_discrete(expand = c(0, 0.3)) +
  geom_boxplot(width=0.5, size=0.3, outlier.size = 0.5) + 
  labs(title="FFA concentrations", x="", fill="", y="Concentration (µM/L)")
ggsave(filename = "Results/FFA_values_con_ZOOM_asse_y_ZOOM_without_2MethButirr.png", height=4.2, width = 6.8, dpi=300)
# poster version
ggplot(data=table_SCFA2, mapping=aes(x=variable, y=value, fill=Condition)) +
  scale_fill_manual(values = c("Healthy"="chartreuse","RCC"="coral")) +
  theme_bw(base_size = 15) +
  theme( axis.text.x = element_text(angle=28, size = 8.6,
                                    vjust=1.05, hjust = 1),
         axis.text.y = element_text(size = 7),
         axis.title.y = element_text(size = 11),
         panel.grid.minor.y = element_line(linewidth = 0.1),
         ...
         legend.margin = margin(-25,0,1,1),
         plot.margin = margin(5,2,2,2)) +
  scale_y_log10(breaks= c(0.1, 0.2, 0.5, 1, 2.5, 5, 10, 25, 50,
                          100, 250, 500)) +
  scale_x_discrete(expand = c(0, 0.3)) +
  geom_boxplot(width=0.8, size=0.3, notchwidth = 0.5,
               outlier.size = 0.45, varwidth = 0.5) + 
  labs(title="Serum FFA concentrations", x="", fill="", y="Concentration (µM/L)")
ggsave(filename = "FFA_values_poster.png", height=3.5, width = 5, dpi=500)
