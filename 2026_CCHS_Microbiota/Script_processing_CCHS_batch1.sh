...
mkdir QIIME_batch1
chmod +rwx QIIME_batch1



##### FASTQ CHECKSUM

md5sum raw_FASTQ_dir_batch1/* | sed 's+raw_FASTQ_dir_batch1/++g' | sed 's/  / \t/' > QIIME_batch1/checksum_FASTQ.tsv

dir raw_FASTQ_dir_batch1/ | sed '/R2_001/d' >R1.txt
cat R1.txt | sed 's/R1_001/R2_001/' >R2.txt
paste R1.txt R2.txt --delimiters='\t' > QIIME_batch1/FASTQ_pairs.tsv
rm R1.txt R2.txt
...
qiime tools import --type 'SampleData[PairedEndSequencesWithQuality]' --input-path ../raw_FASTQ_dir_batch1 --input-format CasavaOneEightSingleLanePerSampleDirFmt --output-path demux-paired-end.qza
...
############# CLEANING THE FOLDER FROM QIIME_batch1 TEMP FILES #####################
