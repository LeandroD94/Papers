...
# CKS_prop[ , c("IFN-gamma","IL17A","IL-10","IL21","IL-5")]<-CKS[ , c("IFN-gamma","IL17A","IL-10","IL21","IL-5")]/rowSums(CKS[ , c("IFN-gamma","IL17A","IL-10","IL21","IL-5")])
# rowSums(CKS_prop[ , c("IFN-gamma","IL17A","IL-10","IL21","IL-5")])
# 
# #### normalization of FFA respect total SCFA, MCFA or LCFA
SCFA_prop<-SCFA
SCFA_prop[ !colnames(SCFA_prop)%in% c("Subject","Condition")]<-SCFA_prop[!colnames(SCFA_prop)%in% c("Subject","Condition")]/rowSums(SCFA_prop[!colnames(SCFA_prop)%in% c("Subject","Condition")])
# 
MCFA_prop<-MCFA
MCFA_prop[ !colnames(MCFA_prop)%in% c("Subject","Condition")]<-MCFA_prop[!colnames(MCFA_prop)%in% c("Subject","Condition")]/rowSums(MCFA_prop[!colnames(MCFA_prop)%in% c("Subject","Condition")])
# 
LCFA_prop<-LCFA
LCFA_prop[ !colnames(LCFA_prop)%in% c("Subject","Condition")]<-LCFA_prop[!colnames(LCFA_prop)%in% c("Subject","Condition")]/rowSums(LCFA_prop[!colnames(LCFA_prop)%in% c("Subject","Condition")])
# rowSums(LCFA[!colnames(LCFA)%in% c("Subject","Condition")])


SCFA_post_prop<-FFA_post[,c(1:9)]
SCFA_post_prop[ !colnames(SCFA_post_prop)%in% c("Subject","Sample_pair_pre_treat")]<-SCFA_post_prop[!colnames(SCFA_post_prop)%in% c("Subject","Sample_pair_pre_treat")]/rowSums(SCFA_post_prop[!colnames(SCFA_post_prop)%in% c("Subject","Sample_pair_pre_treat")])
MCFA_post_prop<-FFA_post[,c(1,2,10:15)]
MCFA_post_prop[ !colnames(MCFA_post_prop)%in% c("Subject","Sample_pair_pre_treat")]<-MCFA_post_prop[!colnames(MCFA_post_prop)%in% c("Subject","Sample_pair_pre_treat")]/rowSums(MCFA_post_prop[!colnames(MCFA_post_prop)%in% c("Subject","Sample_pair_pre_treat")])
LCFA_post_prop<-FFA_post[,c(1,2,16:18)]
LCFA_post_prop[ !colnames(LCFA_post_prop)%in% c("Subject","Sample_pair_pre_treat")]<-LCFA_post_prop[!colnames(LCFA_post_prop)%in% c("Subject","Sample_pair_pre_treat")]/rowSums(LCFA_post_prop[!colnames(LCFA_post_prop)%in% c("Subject","Sample_pair_pre_treat")])
# just a check
rowSums(SCFA_post_prop[!colnames(SCFA_post_prop)%in% c("Subject","Sample_pair_pre_treat")])
...
ggplot(data=table, mapping=aes(x=variable, y=value, fill=Condition)) +
  ...
  theme( axis.text.x = element_text(angle=-25, size = 8, vjust=1, hjust = 0.05),
         axis.ticks.y.right = element_blank(), # to remove the double axis from scale_y_break
         axis.text.y.right = element_blank()) + # to remove the double axis from scale_y_break
  scale_y_continuous(breaks= c(0.38, 2.5, seq(5, 40, 5))) +
  # scale_y_break(c(40, 175), ticklabels = c(0.5, 175, 180, 185, 190)) +
  scale_fill_manual(values = c("HC"="deepskyblue","PV"="coral"))
...
head(table_SCFA, n=2)
table_SCFA$signif<-ifelse(table_SCFA$variable %in% c("Propionic","Butyric","isoButyric"), "*", "")
...
ggplot(data=table_SCFA, mapping=aes(x=variable, y=value, fill=Condition)) +
  ...
  labs(title="SCFA concentration", x="", fill="", y="Concentration (µM/L)")
ggsave(filename = "FFA/SCFA_zoom_plot.png", height=4.5, width = 7, dpi=300)


# specific plot for MCFA
head(table_MCFA, n=2)
unique(table_MCFA$variable)
table_MCFA$signif<-ifelse(table_MCFA$variable %in% c("Hexanoic","Heptanoic","Octanoic"), "*", "")   # DA CAMBIARE!
...
ggsave(filename = "FFA/MCFA_zoom_plot.png", height=4.5, width = 7, dpi=300)


# specific plot for LCFA
head(table_LCFA, n=2)
table_LCFA$signif<-ifelse(table_LCFA$variable %in% c("Hexadecanoic"), "*", "")   # DA CAMBIARE!
...
ggplot(data=table_LCFA, mapping=aes(x=variable, y=value, fill=Condition)) +
  ...
  scale_x_discrete(expand = c(0, 0.4)) +
  geom_boxplot(width=0.65, size=0.5, outlier.size = 1) + 
  labs(title="LCFA concentration", x="", fill="", y="Concentration (µM/L)")
...
for(y in 1:length(list)){
  ...
  PEM<-x[x$Condition=="PV", ! colnames(x) %in% c("Subject","Condition") ] # cutting only values of PV subjects
  HC<-x[x$Condition=="HC", ! colnames(x) %in% c("Subject","Condition") ] # cutting only values of HC subjects
  
  # sub loop to correlate values of every subject, PEM subjects or HC subjects
  sub_list<- list("every"=x[, ! colnames(x) %in% c("Subject","Condition")], "PV"=PEM, "HC"=HC)
                  #1                                                          #2            #3           
...
########### BETWEEN PDAI AND SCFA

CKS_PDAI<-as.data.frame(CKS)
rownames(CKS_PDAI)<-CKS_PDAI$Subject
CKS_PDAI<-CKS_PDAI[rownames(PDAI_CKS), ]
identical(CKS_PDAI$Subject,PDAI_CKS$Subject)
x<-cbind.data.frame(PDAI_CKS, CKS_PDAI)   # the order of the samples has been already checked in the first paragraph
x<-x[, ! colnames(x) %in% c("Subject","PDAI_gravity","Condition")]
...
SCFA_PDAI <- as.data.frame(SCFA_raw)
rownames(SCFA_PDAI) <- SCFA_PDAI$Subject
SCFA_PDAI <- SCFA_PDAI[rownames(PDAI), ]
identical(SCFA_PDAI$Subject,PDAI$Subject)
x<-cbind(PDAI, SCFA_PDAI)   # the order of the samples has been already checked in the first paragraph
x<-x[, ! colnames(x) %in% c("Subject","PDAI_gravity","Condition")]
x<-x[ ! x$PDAI_pre=="NOT_KNOWN", ]

r<-rcorr(as.matrix(x), type = "spearman")
data_corr<-as.data.frame(as.table(r$r))
data_pvalue<-as.data.frame(as.table(r$P))
correlation<-cbind(data_corr,data_pvalue[,3])
colnames(correlation)<- c("PDAI","SCFA","corr","pvalue")
correlation<-correlation[ correlation[[1]] =="PDAI_pre", ]
correlation<-correlation[ correlation[[2]] %in% colnames(SCFA_PDAI), ]
correlation$'padj(BH)'<-p.adjust(correlation$pvalue, method = "BH")
correlation$Sign<-correlation$`padj(BH)`
correlation$Sign[correlation$Sign<0.05]<-"*"
correlation$Sign[correlation$Sign>0.05]<-" "

write.csv2(correlation, file = "Correlations/Spearm_PDAI_vs_SCFA_.csv", row.names=F, quote=F)
...
ggplot(correlation, aes(x = correlation[[1]], y = correlation[[2]], fill = corr)) +
  ...
  geom_text(aes(label= Sign), color= "white", size =4.5) +
  labs(title = "Spearman correlation between",
       subtitle = "PDAI and SCFA raw abundances",
       y= "SCFA", x= "PDAI", caption = "p-adj lower than 0.05 are displayed through * symbol") +
  theme(legend.text = element_text(size=10), legend.key.height= unit(1, 'cm'),
        legend.key.width= unit(0.6, 'cm'), legend.title = element_text(size=10))
ggsave(filename = "Correlations/Heatmap_PDAI_vs_SCFA.png", width = 5, height = 4, dpi= 300 )


########### BETWEEN PDAI AND MCFA

MCFA_PDAI <- as.data.frame(MCFA_raw)
rownames(MCFA_PDAI) <- MCFA_PDAI$Subject
MCFA_PDAI <- MCFA_PDAI[rownames(PDAI), ]
identical(MCFA_PDAI$Subject,PDAI$Subject)
x<-cbind(PDAI, MCFA_PDAI)   # the order of the samples has been already checked in the first paragraph
x<-x[, ! colnames(x) %in% c("Subject","PDAI_gravity","Condition")]
x<-x[ ! x$PDAI_pre=="NOT_KNOWN", ]
...
data_pvalue<-as.data.frame(as.table(r$P))
correlation<-cbind(data_corr,data_pvalue[,3])
colnames(correlation)<- c("PDAI","MCFA","corr","pvalue")
correlation<-correlation[ correlation[[1]] =="PDAI_pre", ]
correlation<-correlation[ correlation[[2]] %in% colnames(MCFA_PDAI), ]
correlation$'padj(BH)'<-p.adjust(correlation$pvalue, method = "BH")
correlation$Sign<-correlation$`padj(BH)`
correlation$Sign[correlation$Sign<0.05]<-"*"
correlation$Sign[correlation$Sign>0.05]<-" "

write.csv2(correlation, file = "Correlations/Spearm_PDAI_vs_MCFA_.csv", row.names=F, quote=F)
...
ggplot(correlation, aes(x = correlation[[1]], y = correlation[[2]], fill = corr)) +
  ...
  geom_text(aes(label= Sign), color= "white", size =4.5) +
  labs(title = "Spearman correlation between",
       subtitle = "PDAI and MCFA raw abundances",
       y= "MCFA", x= "PDAI", caption = "p-adj lower than 0.05 are displayed through * symbol") +
  theme(legend.text = element_text(size=10), legend.key.height= unit(1, 'cm'),
        legend.key.width= unit(0.6, 'cm'), legend.title = element_text(size=10))
ggsave(filename = "Correlations/Heatmap_PDAI_vs_MCFA.png", width = 5, height = 4, dpi= 300 )


########### BETWEEN PDAI AND LCFA

LCFA_PDAI <- as.data.frame(LCFA_raw)
rownames(LCFA_PDAI) <- LCFA_PDAI$Subject
LCFA_PDAI <- LCFA_PDAI[rownames(PDAI), ]
identical(LCFA_PDAI$Subject,PDAI$Subject)
x<-cbind(PDAI, LCFA_PDAI)   # the order of the samples has been already checked in the first paragraph
x<-x[, ! colnames(x) %in% c("Subject","PDAI_gravity","Condition")]
x<-x[ ! x$PDAI_pre=="NOT_KNOWN", ]
...
data_pvalue<-as.data.frame(as.table(r$P))
correlation<-cbind(data_corr,data_pvalue[,3])
colnames(correlation)<- c("PDAI","LCFA","corr","pvalue")
correlation<-correlation[ correlation[[1]] =="PDAI_pre", ]
correlation<-correlation[ correlation[[2]] %in% colnames(LCFA_PDAI), ]
correlation$'padj(BH)'<-p.adjust(correlation$pvalue, method = "BH")
correlation$Sign<-correlation$`padj(BH)`
correlation$Sign[correlation$Sign<0.05]<-"*"
correlation$Sign[correlation$Sign>0.05]<-" "

write.csv2(correlation, file = "Correlations/Spearm_PDAI_vs_LCFA_.csv", row.names=F, quote=F)
...
ggplot(correlation, aes(x = correlation[[1]], y = correlation[[2]], fill = corr)) +
  ...
  geom_text(aes(label= Sign), color= "white", size =4.5) +
  labs(title = "Spearman correlation between",
       subtitle = "PDAI and LCFA raw abundances",
       y= "LCFA", x= "PDAI", caption = "p-adj lower than 0.05 are displayed through * symbol") +
  theme(legend.text = element_text(size=10), legend.key.height= unit(1, 'cm'),
        legend.key.width= unit(0.6, 'cm'), legend.title = element_text(size=10))
ggsave(filename = "Correlations/Heatmap_PDAI_vs_LCFA.png", width = 5, height = 4, dpi= 300 )
...
CKS_PCoA<-as.data.frame(CKS_raw)           # nothing there!!! go to FFA
...
list_PCoA<-list("SCFA"=SCFA_prop,"MCFA"=MCFA_prop,"LCFA"=LCFA_prop) # bray is sensible to different library sizes
                  #1               #2            #3
...
for(x in 1:length(list_PCoA)){
  ...
  ggplot(data=as.data.frame(coord$vectors), aes(x=V1, y=V2, color=data$Condition) ) + 
    ...
    theme_classic() +  stat_ellipse(size=0.2) +
    geom_text(aes(label=row.names(data)),size=2, color="black")+
    labs(x=paste("PC1:",round(coord[["values"]][1]/sum(coord[["values"]])*100,digits=2),"%"),
         y=paste("PC2:",round(coord[["values"]][2]/sum(coord[["values"]])*100,digits=2),"%"),
         color="", title = paste("PCoA computed on",names(list_PCoA)[x],"quantities\n of HC and PV subjects\n with Bray-Curtis dissimilarity"))
  ggsave(file=paste("PCoA/based_on_FFA/PCoA_BRAY_",names(list_PCoA)[x],"HC_PV.png", sep="_"), height = 5, width = 6.5, dpi=300)              
  ...
  ggplot(data=as.data.frame(coord$vectors), aes(x=V1, y=V2, color=data$Condition) ) + 
    ...
    theme_classic() + 
    stat_ellipse(size=0.19) +
    theme( legend.text = element_text(size=11)) +
    labs(x=paste("PC1:",round(coord[["values"]][1]/sum(coord[["values"]])*100,digits=2),"% variance"),
         y=paste("PC2:",round(coord[["values"]][2]/sum(coord[["values"]])*100,digits=2),"% variance"),
         color="", title = paste("PCoA computed on",names(list_PCoA)[x],"quantities\n of HC and PV subjects\n with Bray-Curtis dissimilarity"))
  ggsave(file=paste("PCoA/based_on_FFA/PCoA_BRAY_",names(list_PCoA)[x],"HC_PV_only_points.png", sep="_"), height = 5, width = 6.5, dpi=300)              
  ...
  ggplot(data=as.data.frame(coord$vectors), aes(x=V1, y=V2, color=PDAI_FFA$PDAI_gravity) ) + 
    ...
    labs(x=paste("PC1:",round(coord[["values"]][1]/sum(coord[["values"]])*100,digits=2),"%"),
         y=paste("PC2:",round(coord[["values"]][2]/sum(coord[["values"]])*100,digits=2),"%"),
         color="", title = paste("PCoA computed on",names(list_PCoA)[x],"quantities\n of PV subjects\n with Bray Curtis dissimilarity"))
  ggsave(file=paste("PCoA/based_on_FFA/PCoA_BRAY_",names(list_PCoA)[x],"PV_PDAI.png", sep="_"), height = 5, width = 6, dpi=300)              
}  

p_values$adjusted_BH_p_values<-p.adjust(p_values$`Pr(>F)`, method = "BH")
p_values
write.csv2(p_values, file="PCoA/based_on_FFA/p_values_BRAY_HC_vs_PV.csv", quote = F,row.names = T)
...
# on proportion (for Bray Curtis)
list_PCoA<-list("SCFA"=SCFA_post_prop,"MCFA"=MCFA_post_prop,"LCFA"=LCFA_post_prop)
list_PCoA_pre<-list("SCFA"=SCFA_prop,"MCFA"=MCFA_prop,"LCFA"=LCFA_prop)
                       #1               #2              #3
# 
...
for(x in 1:length(list_PCoA)){
  ...
  temp$Condition<-gsub("Treated","PV_treated" , temp$Condition)
  
  ggplot(data=as.data.frame(coord$vectors), aes(x=V1, y=V2, color=temp$Condition) ) + 
    geom_point(size=2.5, alpha=0.6) + theme_classic(base_size = 11) +  stat_ellipse() +
    # geom_text(aes(label=row.names(temp)),size=1.8, color="black") +
    geom_line(aes(group=temp$Subject_ID), size=0.2, color="grey") +
    scale_color_manual(values=c("HC"="deepskyblue", "PV"="red", "PV_treated"="coral")) +
    theme(legend.text = element_text(size=10)) +
    labs(x=paste("PC1:",round(coord[["values"]][1]/sum(coord[["values"]])*100,digits=2),"%"),
         y=paste("PC2:",round(coord[["values"]][2]/sum(coord[["values"]])*100,digits=2),"%"),
         color="", caption = "lines connect the same subject pre and post treatment",
         title = paste("PCoA computed on",names(list_PCoA)[x],"quantities\n with Bray Curtis dissimilarity"))
  
  ggsave(file=paste("PCoA/FFAs_pre_post_Treatment/PCoA_BRAY",names(list_PCoA)[x],"treatment.png", sep="_"), height = 4.8, width = 6.5, dpi=300)              
  
  # checking for significant dispersion
  rm(p_value_diver)
  p_value_diver<- pairwise.adonis(dist, factors=temp$Condition, p.adjust.m = "BH", sim.method="bray", perm = 9999)
  write.csv2(p_value_diver, file=paste("PCoA/FFAs_pre_post_Treatment/Beta_diversity_",names(list_PCoA)[x],"HC_PEM_treat.csv"), quote=F, row.names=F)
}
...
perf.splsda <- perf(my.splsda, validation = "Mfold", # to assest the optimal number of comp
                    folds = 10, nrepeat = 5*(dim(X)[1]), cpus = 8,
...
ggplot(mapping = aes(x=final.splsda$variates$X[,"comp1"], y=final.splsda$variates$X[,"comp2"], color=final.splsda$Y)) +
  geom_point(size=3.2) + theme_classic(base_size = 13) + stat_ellipse() +
  geom_text(mapping = aes(label=row.names(final.splsda$X)), color="black", size=2) +
  scale_colour_manual(values = c("HC" = "deepskyblue", "PV" = "coral"))+
  labs(title="sparse PLS-DA between HC and PV subjects", 
...
ggplot(mapping = aes(x=final.splsda$variates$X[,"comp1"], y=final.splsda$variates$X[,"comp2"], color=final.splsda$Y)) +
  geom_point(size=2.38, alpha= 1) +
  geom_point(size=3.8, alpha= 0.65) +
  theme_classic(base_size = 13) + stat_ellipse(linewidth=0.2) +
  scale_colour_manual(values = c("HC" = "deepskyblue", "PV" = "coral"))+
  labs(title="sparse PLS-DA between HC and PV subjects", 
       subtitle=paste0(" on raw FFAs abundances \n  (computed with ", optimal.comp, " components, plotted on comp 1 and ", n,")"),
       caption = paste0("selected ",optimal.keepX[1]," FFAs on LC1 and ",optimal.keepX[2]," FFAs on LC2"),
       ...
       y=paste("LC2: ",round(final.splsda$prop_expl_var$X[2]*100,digits = 2),"% variance"))
ggsave(filename = paste("PLS_DA/sPLS-DA on comp 1 and",n,"_points_only.png"), width = 6.5, height = 5, dpi=300) 
dev.off()
...
rm(con)
con<-file("PLS_DA/Valori_parametri_risultati_prediz_di_sPLSDA.txt")
sink(con)
...
########## EXTRA: CHECKING THE CORRELATIONS BETWEEN ILL DURATION ANS FFAs ###############

dir.create("extra_check")

duration<-read.table("../Durata_malattia_costruito_da_db_29012024.tsv", sep="\t", header = T)
duration <- duration [! is.na(duration$Durata_malattia_al_1o_prelievo) , ]
duration$Pazienti<-gsub("/","_",duration$Pazienti)
row.names(duration)<-duration$Pazienti
duration$Pazienti<-NULL


rm(list)
list<-list("SCFA"=SCFA, "MCFA"=MCFA, "LCFA"=LCFA)
                #1         #2            #3

for(y in 1:length(list)){
  x<-as.data.frame(list[[y]])
  row.names(x)<-x$Subject
  x<-x[ , ! colnames(x) %in% c("Subject","Condition")]
  x<-x[ row.names(x) %in% row.names(duration), ]
  duration<-duration[ row.names(duration) %in% row.names(x), , drop=F ]
  x<-x[ row.names(duration), ]
  
  r<-rcorr(as.matrix(cbind(x,duration)), type = "spearman")
  data_corr<-as.data.frame(as.table(r$r))
  data_pvalue<-as.data.frame(as.table(r$P))
  correlation<-cbind(data_corr,data_pvalue[,3])
  colnames(correlation)<-c(paste(names(list)[y]),"Duration","Corr","pvalue")
  correlation<-correlation[ correlation[[1]] %in% colnames(list[[y]]), ]
  correlation<-correlation[ correlation$Duration %in% colnames(duration), ]
  correlation$'padj(BH)'<-p.adjust(correlation$pvalue, method = "BH")
  correlation$Sign<-correlation$`padj(BH)`
  correlation$Sign[correlation$Sign<0.05]<-"*"
  correlation$Sign[correlation$Sign>0.05]<-" "
  write.csv2(correlation, file = paste0("extra_check/Spearm_",names(list)[y]," vs Illness Duration.csv"), row.names=F, quote=F)
  ...
  ggplot(correlation, aes(x = correlation[[1]], y = correlation[[2]], fill = Corr)) +
    ...
    geom_text(aes(label= Sign), color= "white", size =4.5) +
    labs(title = "Spearman correlation between",
         subtitle = paste0(names(list)[y],"s and Illness Duration"),
         y= "Illness duration",
         x= paste(names(list)[y]), caption = "p-adj lower than 0.05 are displayed through * symbol") +
    theme(legend.text = element_text(size=10), legend.key.height= unit(1, 'cm'),
          legend.key.width= unit(0.6, 'cm'), legend.title = element_text(size=10))
  ggsave(filename = paste0("extra_check/Heatmap_",paste(names(list)[y]," vs Illness Duration.csv.png")), width = 5, height = 4, dpi= 300 )
}
